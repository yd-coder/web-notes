...
2. `vite-plugin-eslint`可在控制台打印错误信息，安装后在`vite.config.js`中注册插件
...
3. 使用scss和scss lint，需要在IDE stylelint加上支持scss文件
...
3. main.ts里引入element-plus的base.css
...
2. 分别在项目根目录建立`.env.development`,`.env.production`,`.env.test`文件
...
1. 安装插件
   ...
   // 此处如果vite运行npm run dev时报Cannot find module 'fast-glob'的错误安装下fast-glob相关依赖
...
3. 封装一个SvgIcon组件，组件可以提供name、color、className三个属性，分别对应图标名称、颜色、以及自定的css类名，有了这个组件，我们就可以方便的引用对应的svg，而不必写大量的重复代码。
...
6. SvgIcon组件的使用
   ...
   ```vue
   <template>
     <svg-icon name="category" color="red"></svg-icon>
   </template>
   ```
...
[PostCSS - 是一个用 JavaScript 工具和插件来转换 CSS 代码的工具 | PostCSS 中文网](https://www.postcss.com.cn/)，常见的autoprefixer，tailwindcss，stylelint都需要postcss支持
