...
1. 打开xshell新建连接
2. 填好服务器的外网ip和用户名密码连接
3. 连接成功后到宝塔官网复制Centos安装脚本安装宝塔
4. 打开安装好后提供的宝塔外网面板填好账号密码登录
5. 数据库导入sql文件
6. 安全添加8889，3306两个端口
7. 文件到wwwroot目录下上传后端代码压缩包解压到当前目录
8. 编辑后端代码里的数据库，启动端口号8889配置
9. 软件商店下载pm2托管node后端服务
10. 安装好后pm2需要切换好自己后端的node版本和启动目录下的后端app.js文件
11. 前端代码打包dist文件夹压缩上传到面板文件的wwwroot/http目录下
12. 解压后剪切里面文件到http目录下
13. 网站添加站点将外网ip部署起来
